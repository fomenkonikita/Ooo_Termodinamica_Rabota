...
def ensure_dashboard_employee_rows():
    ...
    for i in range(cap1):
        ...
        if i < n:
            ...
            # Прогул (лист "Прогулы") — приоритет над Журналом, иначе самообъявленный
            # выходной показывает "Не на месте" вместо "Прогул" (найдено 10.07.2026:
            # с 03.07 этот статус вообще не проверял Прогулы, только Журнал).
            V[f"G{r}"] = (f'=IF(COUNTIFS(Прогулы!$A$2:$A$500;TODAY();Прогулы!$B$2:$B$500;"{nm}")>0;"Прогул";'
